...
class Model_1(multiprocessing.Process):
    ...
    def run(self):
        ...
        while True:
            ...
            with model_1_lock:
                ...
                if request == -1:
                    self.input_video_paths_list.put(-1) # put the end signal back
                    if self.car_frames_list.qsize() == 0 and self.person_frames_list.qsize() == 0:
                        ...
                        if self.end_signal.value == 0:
                            self.car_frames_list.put(-1) # TODO: 特殊信号 -> Request
                            self.person_frames_list.put(-1)
                        break
                    else:
                        continue
    ...
    def process_video(self, request):
        ...
        self.car_frames_list.put(int(input_video_path.split('_')[-1].split('.')[0]))
        self.person_frames_list.put(int(input_video_path.split('_')[-1].split('.')[0]))
